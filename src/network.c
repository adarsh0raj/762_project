...
struct network* generate_random_network(struct network_params net_params, gsl_rng* random_generator){
  ...
  struct node* node1 = new_node(node_id_counter);
  struct node* node2 = new_node(node_id_counter+1);
  network->nodes = array_insert(network->nodes, node1);
  network->nodes = array_insert(network->nodes, node2);

  for(i=0; i<net_params.n_nodes; i++){
    double prob = gsl_ran_flat(random_generator, 0.0, 1.0);
  ...
  long double init_channel = channel_id_counter;
  ...
  init_balance = 1000.0 * (channel_id_counter - init_channel) * net_params.capacity_per_channel;

  printf("Initial Balance of Attacker: %Lf\n", init_balance);

  node_id_counter += 2;

  attacker_id = node_id_counter - 2;

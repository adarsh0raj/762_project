...
uint64_t tot_flows_max = 50;

long double prob = 0.5;
double threshold = 0.95;
extern long double init_balance;
...
void send_payment(struct event* event, struct simulation* simulation, struct network *network) {
  ...
  // if(next_edge->tot_flows > tot_flows_max) {
  //   printf("ERROR: Maximum HTLCs per edge exceeded\n");
  //   exit(-1);
  // }
...
void forward_success(struct event* event, struct simulation* simulation, struct network* network) {
  ...
  if(prev_node_id != attacker_id || prob > threshold)
  {
    backward_edge->balance += prev_hop->amount_to_forward;
    init_balance = init_balance + prev_hop->amount_to_forward;
  }
